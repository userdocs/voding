		...
		debian | */debian | ubuntu | */ubuntu)
			...
			printf '%b\n' "\`\`\`bash\n" >> "$GITHUB_STEP_SUMMARY"
			{
				...
				df_lines=(
					...
					"    apt-get install -y sudo${inputs_additional_debian_apps_array[*]:+ ${inputs_additional_debian_apps_array[*]}} && ${backslash}"
		...
		*)
			...
			printf '%b\n' "\`\`\`bash\n" >> "$GITHUB_STEP_SUMMARY"
			{
				...
				df_lines=(
					...
					"    apt-get update >/dev/null 2>&1 && apt-get install -y sudo${inputs_additional_debian_apps_array[*]:+ ${inputs_additional_debian_apps_array[*]}} >/dev/null 2>&1; ${backslash}"
					"  elif command -v apk >/dev/null 2>&1; then ${backslash}"
					"    apk add --no-cache sudo${inputs_additional_alpine_apps_array[*]:+ ${inputs_additional_alpine_apps_array[*]}} >/dev/null 2>&1; ${backslash}"
					"  fi && ${backslash}"
